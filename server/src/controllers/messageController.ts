...
export const getConversations = async (req: Request, res: Response) => {
  ...
  const conversations = await em.getConnection('read').execute(
    `
      ...
      from conversations c
			left join messages m on c."id" = m."conversation_id" and m."id" in (select id from messages order by messages."created_at" desc limit 1)
  ...
  console.log('conversatoin', conversations);
  const myMap: Record<string, any> = {};
  conversations.forEach((c) => {
    const isOtherOne = c.owner_one_id !== userId ? true : false;
    myMap[c.id] = {
      id: c.id,
      lastActivity: c.last_activity,
      other: {
        id: isOtherOne ? c.owner_one_id : c.owner_two_id,
        username: isOtherOne ? c.owner_one_username : c.owner_two_username,
        isOnline: isOtherOne ? c.owner_two_is_online : c.owner_two_is_online,
        lastLoginAt: isOtherOne
          ? c.owner_two_last_login_at
          : c.owner_two_last_login_at,
      },
      latestMsg: {
        id: c.message_id,
...
export const getMessages = async (req: Request, res: Response) => {
  ...
  console.log(
    'conversationId at get all messages route ',
  ...
  await em.findOneOrFail(Conversation, {
    $or: [
      { ownerOne: em.getReference(User, userId!) },
      { ownerTwo: em.getReference(User, userId!) },
    ],
    id: conversationId,
  });
  let replacements = [];
  ...
  console.log('messages at messages route ', messages);
